...
- (void)saveInstallationWithDeviceToken:(NSData *)deviceToken {
    ...
    currentInstallation.deviceProfile = @"renaihealth_servicer_client";
    ...
    if([AppDelegate getCurrentLogonUser].userId){
        [currentInstallation addUniqueObject:[NSString stringWithFormat:@"servicer_%@",[AppDelegate getCurrentLogonUser].userId ] forKey:kAVIMInstallationKeyChannels];
    }
