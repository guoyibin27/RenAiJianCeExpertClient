...
- (void)fetchConvWithOwner:(NSString *)owner callback:(AVIMConversationResultBlock)callback
{
    if(nil == owner || owner.length == 0){
        [NSException raise:NSInvalidArgumentException format:@"owner shold not be nil"];
    }
    AVIMConversationQuery *q = [[AVIMClient defaultClient] conversationQuery];
    [q whereKey:kAVIMKeyCreator equalTo:owner];
    q.limit = 1;
    [q findConversationsWithCallback:^(NSArray *objects, NSError *error) {
        if(error){
            callback(nil,error);
            ...
            if(objects.count == 0){
                callback(nil,[CDChatManager errorWithText:[NSString stringWithFormat:@"conversation of %@ not exists",owner]]);
            }else{
                callback([objects objectAtIndex:0], error);
            }
        }
...
- (void)quiteConversation:(AVIMConversation *)conv block:(AVBooleanResultBlock)block{
    [conv quitWithCallback:block];
}

- (void)joinConversation:(AVIMConversation *)conv callback:(AVIMBooleanResultBlock)callback{
    [conv joinWithCallback:callback];
}

- (void)createConvWithOwner:(NSString *)owner name:(NSString *)name callback:(AVIMConversationResultBlock)callback
{
    if(nil == owner || owner.length == 0){
        [NSException raise:NSInvalidArgumentException format:@"owner shold not be nil"];
    }
    [[AVIMClient defaultClient] createConversationWithName:name clientIds:@[owner] attributes:@{ CONV_TYPE:@(CDConvTypeGroup) } options:AVIMConversationOptionNone callback:callback];
}

- (void)removeMembersWithClientIds:(NSArray *)clientIds conversation:(AVIMConversation *)conv block:(AVBooleanResultBlock)block{
    [conv removeMembersWithClientIds:clientIds callback:block];
}
@end
